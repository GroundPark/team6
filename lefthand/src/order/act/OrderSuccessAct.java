...
public class OrderSuccessAct implements Action {
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
	// 앞에 public 붙여야 함. 오버라이딩. 접근제어지시자는 무조건 원본보다 같거나 커야함. 원본이 public임
		request.setCharacterEncoding("utf-8");
		String poiid = request.getParameter("poiid");	// 쿼리스트링으로 주문번호를 받아옴

		response.setContentType("text/html; charset=utf-8");
		PrintWriter out = response.getWriter();
		if (poiid == null) {	// 보여줄 주문의 주문번호가 없을 경우
		...
		OrderInfo ord = orderSuccessSvc.getOrderInfo(memberInfo.getMi_id(), poiid);
		if (ord == null) {
			out.println("<script> alert('잘못된 경로로 들어오셨습니다.'); history.back(); </script>");
			out.close();			
		}
		
		request.setAttribute("ord", ord);	// 주문에 대한 정보들을 저장한 인스턴스를 request 객체의 속성으로 저장
		ActionForward forward = new ActionForward();	// 작업 후 이동할 위치와 방법에 대해 지정하는 ActionForward 인스턴스 생성
		forward.setPath("/page/order/pdt_order_success.jsp");	// 디스패치로 이동
